...
export const generateStaticParams = () => {
  ...
  const paths = Object.keys(seriesList).map((seriesItem) => ({
    series: slug(seriesItem),
  }));
  return paths;
};
    ...
    <div className="container max-w-4xl py-6 lg:py-10">
      ...
      <div className="grid grid-cols-12 gap-8 mt-8">
        ...
        <div className="col-span-12 row-start-3 h-fit sm:col-span-4 ml-0 sm:ml-6 sm:col-start-9 sm:row-start-1">
          <p className="text-slate-600 dark:text-slate-300 text-xl font-light capitalize mb-4">
            daftar series
          </p>
          <div className="flex flex-col gap-6">
            {sortedSeries?.map((seriesItem) => (
              <div key={seriesItem.name}>
                <Link
                  href={`/collections/series/${slug(seriesItem.name)}`}
                  className="text-slate-600 dark:text-slate-300 mb-1"
                >
                  {seriesItem.name}
                </Link>
                <p className="text-sm text-muted-foreground">
                  {seriesItem.count} post
                </p>
              </div>
            ))}
          </div>
